...
<p>Once the server minimum and maximum server memory has been set, restart the SQL server by right clicking LABCM01 and selecting “Restart”. The SQL server is now ready to host the Configuration Manager database. In the next post we will be  <a href="">installing and configuring all the Configuration Manager prerequisites</a>.</p>
